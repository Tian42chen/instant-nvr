...
def main():
    ...
    print(f"[INFO] Processing {data_root}...")
    ...
    print("[INFO] Converting vertices and params...")
    ...
    print("[INFO] Vertices and params done.")

    # annots
    annots_path=os.path.join(data_root, 'annots.npy')
    print("[INFO] Converting annots...")
    ...
    try:
        ...
        print('[WARN] Camera info error, use default camera')
        imgname = image_list[0]
        img_path = os.path.join(data_root, imgname)
        img = cv2.imread(img_path)
        height, width = img.shape[:2]
        print('[INFO] Read image shape {}'.format(img.shape))
        focal = 1.2*min(height, width) # as colmap
        print('[INFO] Set a fix focal length {}'.format(focal))
